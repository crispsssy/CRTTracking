...
void CalibInfo::ReadXTTableIteration(int const itr_index){
    std::cout<<"Start reading iterated XT file"<<std::endl;
    auto store = gFile;
    ...
    std::string f_XT_path;
#endif
    std::string f_XT_name = f_XT_path + Form("/xt_run%05d/itr%d/xt.root", runNum, itr_index);
    std::unique_ptr<TFile> f_XT { TFile::Open(f_XT_name.c_str(), "READ") };
    if(!f_XT){
        std::string msg = "Failed to open XT file " + f_XT_name;
        throw std::runtime_error(msg);
    }

    TIter next(f_XT->GetListOfKeys());
    TKey* key;
    while( (key = (TKey*)next()) ){
        TGraph* g_in = dynamic_cast<TGraph*>(key->ReadObj());
        if(g_in){
            std::string graphName = g_in->GetName();
            if(graphName == "g_XTSimple"){
                fSimpleXT.reset(g_in);
            }
            else if(graphName == "g_timeResolution68"){
                fSimpleReso.reset(g_in);
            }
            else continue;
        }
    }
...
void CalibInfo::GenerateSimpleXT(){
    ...
    fSimpleXT = std::make_shared<TGraph>();
    fSimpleReso = std::make_shared<TGraph>();
    double const shift = 2.3;
    double const angle = 0.393; //22.5 degrees
    for(int iDoca = 0; iDoca < 95; ++iDoca){
        double doca = iDoca * 0.1;
        fSimpleXT->AddPoint(doca, GetTAtXYShift(doca * cos(angle), doca * sin(angle), shift));
        if(iDoca < 75){
            fSimpleReso->AddPoint(doca, GetTimeResolution(doca * cos(angle), doca * sin(angle), shift));
        }
    }
/*  
    TFile f("graph.root", "RECREATE");
    fSimpleXTGraph->SetName("fSimpleXT");
    fSimpleResoGraph->SetName("fSimpleReso");
